# Issue 08 — Courses — Discovery & Detail Vertical Slice
...
Read course list/detail dari Moodle untuk TENANT dan STUDENT sesuai tenant/enrolment access.
...
src/app/api/courses/
src/app/(protected)/dashboard/courses/
src/app/(protected)/dashboard/courses/[id]/
```
...
- [ ] Semua port di satu `domain/interfaces/CoursesInterfaces.ts`.
- [ ] Implement `GetMyCoursesUseCase` beserta tests.
- [ ] Implement `GetCourseDetailUseCase` beserta tests.
- [ ] Implement `GetCourseContentsUseCase` beserta tests.
...
- Contract mapping: core_enrol_get_users_courses / core_course_* hanya di infrastructure

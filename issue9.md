# Issue 09 — Quizzes — Listing & Access Vertical Slice
...
Read quiz/exam listing, detail, dan access state tanpa mengambil alih Quiz Engine Moodle.
...
- [ ] Semua port di satu `domain/interfaces/QuizzesInterfaces.ts`.
- [ ] Implement `GetCourseQuizzesUseCase` beserta tests.
- [ ] Implement `GetQuizDetailUseCase` beserta tests.
- [ ] Implement `GetQuizAccessUseCase` beserta tests.
...
- Quiz mutation/composition; itu milik exam-administration.
...
- Contract mapping: mod_quiz_get_quizzes_by_courses dan access function yang benar-benar tersedia diverifikasi terhadap backend contract

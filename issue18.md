# Issue 18 — Tenant Branding Vertical Slice
...
Mengelola branding tenant dari SaaS metadata store dengan tenant isolation.
...
src/sections/branding/
src/app/api/branding/
src/app/(protected)/dashboard/branding/
```
...
- [ ] Semua port di satu `domain/interfaces/BrandingInterfaces.ts`.
- [ ] Implement `GetBrandingUseCase` beserta tests.
- [ ] Implement `UpdateBrandingUseCase` beserta tests.
...
- Contract mapping: Prisma metadata only; no Moodle academic duplication
